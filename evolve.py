...
class EvolveCNN(object):
    ...
    def crossover_and_mutation(self):
        cm = CrossoverAndMutation(self.params['genetic_prob'][0],
                                  self.params['genetic_prob'][1],
                                  Log,
                                  self.pops.individuals,
                                  _params={'gen_no': self.pops.gen_no})
    ...
    def do_work(self, max_gen):
        ...
        if StatusUpdateTool.is_evolution_running():
            ...
            if gen_no is not None:
                ...
                raise ValueError(
                    'The running flag is set to be running, but there is no generated population stored'
                )
        else:
            # 显示未建立种群，则建立种群，种群中的个体为根据种群限制随机初始化
            gen_no = 0
            Log.info('Initialize...')
